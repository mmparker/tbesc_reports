...
# Create a flag for those who have had sufficient time to complete treatment
# This doesn't yet account for multiple regimens per person
regimens$enough_time <- FALSE

regimens$enough_time[regimens$AcceptTreatment &
                     regimens$plan %in% "Daily INH" &
                     (Sys.Date() - regimens$OfferDate > (9 * 31))] <- TRUE
    
regimens$enough_time[regimens$AcceptTreatment &
                     regimens$plan %in% "Daily RIF" &
                     (Sys.Date() - regimens$OfferDate > (4 * 31))] <- TRUE

regimens$enough_time[regimens$AcceptTreatment &
                     regimens$plan %in% "Weekly INH/Rifapentine" &
                     (Sys.Date() - regimens$OfferDate > (12 * 7))] <- TRUE

# Don't know how long this should really go on... not more than a year, though.
regimens$enough_time[regimens$AcceptTreatment &
                     regimens$plan %in% "Twice-weekly INH" &
                     (Sys.Date() - regimens$OfferDate > 365)] <- TRUE
...
regimen_table <- ddply(subset(regimens, AcceptTreatment), 
    ...
    per_completed = calc_percent(sum(TreatmentComplete[enough_time], na.rm = TRUE),
                                 sum(enough_time),
                                 digits = 1)
...
# Nicer names
names(regimen_table) <- c("Regimen", "N Started", "% of Starts", "% Completed")
...
Note: treatment completion figures calculated using only participants whose 
regimens have had sufficient time to be completed (e.g., nine months 
for daily INH).
...
result_tx  <- ddply(tx_test_melt, 
    ...
    n_completed = sum(TreatmentComplete[enough_time], na.rm = TRUE),
    per_completed = calc_percent(n_completed,
                                 sum(AcceptTreatment[enough_time], na.rm = TRUE),
                                 digits = 2)
