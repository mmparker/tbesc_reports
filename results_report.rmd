...
# Aggregate by age
testage.agg <- ddply(testage, .var = "AgeAtEnrollment", .fun = summarise,

    n = length(StudyID),
    per.tstpos = sum(tst %in% "Positive") / n * 100,
    per.qftpos = sum(qft %in% "Positive") / n * 100,
    per.tspotpos = sum(tspot %in% "Positive") / n * 100
                     
)

# Melt it for plotting
testage.melt <- melt(testage.agg, id.vars = c("AgeAtEnrollment", "n"))

# Pretty labels
levels(testage.melt$variable) <- c("Percent TST+", 
                                   "Percent QFT+",
                                   "Percent TSPOT+")


ggplot(testage.melt, 
       aes(x = AgeAtEnrollment, 
           y = value)) +
    geom_point(aes(size = n)) +
    geom_smooth(method = "loess", formula = y ~ x) +
    facet_wrap( ~ variable, nrow = 3) +
    coord_cartesian(x = c(0, 50)) +
    labs(x = "Age at Enrollment", y = "% Positive") +
...
monthcount <- ddply(monthmelt,
    ...
    n = length(value),
    per.pos = sum(value %in% "Positive") / n * 100,
    per.bord = sum(value %in% "Borderline") / n * 100,
    per.ind = sum(value %in% "Indeterminate") / n * 100
...
ggplot(monthcount, aes(x = EnrollMonth, 
    ...
    scale_color_manual("Test", 
                       values = c( "#F03B20", "#31A354", "#1F78B4")) +
    scale_linetype_discrete("Test") +
    scale_size_continuous("# Enrolled") +
    labs(x = "Enrollment Month", y = "% Positive") +
    theme_bw() + 
    theme(axis.text.x = element_text(angle = 70, 
                                     vjust = 1.3,
                                     hjust = 1.3),
          legend.key.width = unit(.75, "inches")
    )
